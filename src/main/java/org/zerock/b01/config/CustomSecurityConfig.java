...
@Log4j2
@EnableMethodSecurity(prePostEnabled = false) // 시큐리티 6에서 추가
public class CustomSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        // 강제로그인 화면을 안하게 설정 // board/list -> login
        ...
        // 로그인 창 수동설정
        // 5버전 이하코드 http.formLogin();
        http.formLogin(
                form ->{
                    form.loginPage("/member/login").defaultSuccessUrl("/board/list");
        ...
        // http.csrf().disable(); -> 5버전 이하 코드
        http.csrf(httpSecurityCsrfConfigurer -> httpSecurityCsrfConfigurer.disable()); // 6버전 셋팅
        // 정식 발행 시 토큰을 사용해야 보안상 좋다 -> 각 html파일에 csrf 토큰을 전달해서 해결해야함
    ...
    public WebSecurityCustomizer webSecurityCustomizer(){
        // css나 js 파일에 필터가 적용되면 안됨.수동설정

        log.info("CustomSecurityConfig.webSecurityCustomizer 메서드 실행...");
        log.info("js나 css와 같은 정적페이지에 대한 시큐리티 비활성화");

        return (web) -> web.ignoring().requestMatchers(PathRequest.toStaticResources().atCommonLocations());
        // http://localhost:8080/css/styles.css 정적 경로가 보이기 시작함
    }
